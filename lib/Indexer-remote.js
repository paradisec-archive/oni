...
class Indexer {
  ...
  constructor({configuration, client}) {
    ...
    this.token = this.getOauthToken({key: this.configuration.api.structural.key, secret: this.configuration.api.structural.secret})
  }
  ...
  async getOauthToken({key, secret}) {
    try {
    ...
    for (let obj of ocflObjects) {
      ...
      if (skipRegExp && skipRegExp.test(obj.crateId)) {
        ...
        if (objJSON.status === 500) {
          ...
          const creativeWork = objJSON['@graph'].find((element) => element['@id'] === 'ro-crate-metadata.json')
          const id = creativeWork.about['@id'];
          const url = new URL(id);
          const newId = url.searchParams.get('id')
          creativeWork.about['@id'] = newId;
